...
//! user auth token
  ...
  void setUserCompany({required String company}) {
    log("setYserCompany in hive = $company");
    box.put("userCompany", company);
  }

  String? getUserCompany() {
    return box.get("userCompany");
  }

  void setRegion({required String region}) {
    log("setUserRegion in hive = $region");
    box.put("userRegion", region);
  }

  String? getUserRegion() {
    return box.get("userRegion");
  }
...
//! push notificatons device id
  ...
  Future<void> setFirstInteraction(bool firstTime) {
    return box.put('firstInteraction', firstTime);
  }

  bool? getFirstInteraction() {
    return box.get("firstInteraction");
  }
